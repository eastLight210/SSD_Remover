...
CLI 모드는 그래픽 관리자 인증 창을 열지 않습니다. 선택 대상에 root 소유 프로세스가
포함되면 명령을 명시적으로 `sudo`로 다시 실행해야 하며, 그렇지 않으면 해당 대상은 즉시
실행 가능한 안내 오류를 반환합니다. GUI 모드에서는 관리자 인증을 요청할 수 있습니다.
...
`lsof`, `diskutil`, `kill` 같은 외부 명령에는 30초의 기본 제한 시간이 있어 응답하지 않는
볼륨 때문에 자동화가 영원히 대기하지 않습니다. 타임아웃은 명령 이름과 제한 시간을 포함한
오류 및 0이 아닌 종료 코드를 반환합니다. SSD Remover는 자식 프로세스 종료를 요청하고,
필요하면 강제 종료한 뒤 출력 파이프를 닫습니다. 인터럽트 불가능한 커널 작업에 막힌
프로세스는 정리 요청 후에도 macOS에 잠시 남아 보일 수 있습니다.
